...
function hsgxia2shxurxu(nstr,nwbk)
{
	...
	<body>
			<form action="addGongwenxinxi.do" name="form1" method="post">
				      <table width="100%" border="1" align="center" cellpadding="3" cellspacing="1"  style="border-collapse:collapse" bgcolor="#F2FDFF">
						<tr bgcolor="#E7E7E7">
							<td height="20" colspan="2" background="images/table_header.gif">添加公文信息</td>
						</tr>
...
<script language=javascript >  
function hsgcheck() {
		var gongwenbianhao = $("#gongwenbianhao").val();
		if(gongwenbianhao==""||(gongwenbianhao.length<6||gongwenbianhao.length>12)){
			 $("#clabelgongwenbianhao").html("<font color=red>公文编号不能为空且长度在3～12位之间！</font>");
			 $("input[id=gongwenbianhao]").focus();
			 $("#querenzhuce").attr("disabled","disabled");
			 return false;
		} 
		else
		{
			$("#clabelgongwenbianhao").html("");
			$.ajax({
				url : "quchongGongwenxinxi.do",
				type : "post",
				data : "gongwenbianhao=" + gongwenbianhao,
				dataType : "json",
				success:function(result){
				if(result.info=="ng"){
					$("#clabelgongwenbianhao").html("<font color=red>公文编号已存在，请更换！</font>");
					$("input[id=gongwenbianhao]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
				...
				error:function(){
					$("#clabelgongwenbianhao").html("系统异常，请检查错误！");
					$("input[id=gongwenbianhao]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
