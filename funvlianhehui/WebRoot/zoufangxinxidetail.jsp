		 ...
		 <td width='11%'>标题：</td><td width='39%'>${zoufangxinxi.biaoti}</td>     <td width='11%'>时间：</td><td width='39%'>${zoufangxinxi.shijian}</td></tr><tr>     <td width='11%'>地点：</td><td width='39%'>${zoufangxinxi.didian}</td>     <td width='11%'>走访人员：</td><td width='39%'>${zoufangxinxi.zoufangrenyuan}</td></tr><tr>     <td width='11%'>详情：</td><td width='39%'>${zoufangxinxi.xiangqing}</td>     <td width='11%'>发布人：</td><td width='39%'>${zoufangxinxi.faburen}</td>     </tr><tr>							 
                                <td colspan="4"><table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF" style="border-collapse:collapse" class="newsline">
								  ...
								  <%@page import="com.util.db"%>
                                   <%
								   int pli=0;
    for(HashMap map:new db().getpinglun("zoufangxinxi",id)){
	pli=pli+1;
     %>
                                   <tr>
                                ...
                                </tr><tr><td colspan=4 align=center><input type=button name=Submit5 value=返回 onClick="javascript:history.back()" /><input type=button name=Submit52 value=打印 onClick="javascript:window.print()"  /> <!--jixaaxnnxiu--><input type=button name=Submit53 value=评论 onClick="javascript:hsgpinglun('zoufangxinxi','<%=id%>')"/><input type=button name=Submit52 value=收藏 onClick="javascript:location.href='jrsc.jsp?id=<%=id%>&biao=zoufangxinxi&ziduan=biaoti';"  /></td></tr>
